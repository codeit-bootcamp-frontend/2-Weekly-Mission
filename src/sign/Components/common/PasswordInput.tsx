...
const PasswordInput = ({setPassword} : {setPassword : Dispatch<SetStateAction<string>>}) => {
  ...
  return(
    <div className={style.box}>
      ...
      <div>
        <input type="password" id="password" className={`${style.input} ${(checkEmpty || !checkPassword) ? style['input-error']:''}`} placeholder='영문, 숫자를 조합해 8자 이상 입력해 주세요.' onBlur={handleInputFocusOut} onFocus={handleInputFocusIn}/>
        <EyeButton />
      </div>
