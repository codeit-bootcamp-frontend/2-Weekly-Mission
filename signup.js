window.onload = function(){
    ...
    const verifyPw = function(e){
        const value = e.target.value;
        value !== inputList[1].value ? incorrectVerifyPw() : {};
    }

    const incorrectVerifyPw = function(){
        const massage = document.createElement('small');
        massage.textContent = '비밀번호가 일치하지 않아요.';
        massage.style.color = 'red';
        const signBox = document.querySelectorAll('.sign-input-box')[2];
        inputList[2].style.border = '1px solid red';
        signBox.appendChild(massage);
    }

    inputList[0].addEventListener('focusout', checkEmail);
    inputList[1].addEventListener('focusout', checkPw);
    inputList[2].addEventListener('focusout', verifyPw);
